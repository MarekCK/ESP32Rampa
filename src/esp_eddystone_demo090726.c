...
//        
        ...
        // bike_data[0] = 0x64;
        // bike_data[1] = 0x02;
        // uint16_t cadence = xcadey_cadence * 2;
        // bike_data[4] = cadence & 0xff;
        // bike_data[5] = cadence >> 8;
        // bike_data[8] = xcadey_power & 0xff;
        // bike_data[9] = xcadey_power >> 8;
 ...
 * FTMS TEST MODE
 *
 * Po naciśnięciu BOOT przechodzi do nieskończonej pętli
 * wysyłającej sztuczną moc i kadencję.
 *
 * Używane podczas uruchamiania FTMS i testów MyWhoosh.
 *
 * Pozwoliło zweryfikować:
 *  - połączenie FTMS
 *  - Indoor Bike Data (2AD2)
 *  - Cadence
 *  - Power
 *  - ERG Target Power (0x05)
 *
 * Nie jest używane w finalnej wersji mostka
 * Elite Rampa <-> FTMS.
 *   int btn = gpio_get_level(BOOT_PIN);
    ...
    if (btn == 0 && last_btn == 1) {
        while(1) {
            ...
            bike_data[0] = 0x64;
            bike_data[1] = 0x02;
            // static uint16_t fake_power = 135;

            // cadence 
            uint16_t cadence = 79 * 2;
            bike_data[4] = cadence & 0xff;
            bike_data[5] = cadence >> 8;

            // power 
            bike_data[8] = byte_1; //fake_power & 0xff;
            bike_data[9] = byte_2; //fake_power >> 8;
            ...
            esp_ble_gatts_send_indicate(
                ...
                training_status,
                false);
            // printf("SEND bike data conn=%d\n", conn_id);

            //esp_err_t err = 
            esp_ble_gatts_send_indicate(ftms_gatts_if, ftms_conn_id,
                ftms_handle_table[IDX_CHAR_VAL_BIKE_DATA], sizeof(bike_data), bike_data, false);
            // printf("bike notify err=%d\n", err);
        }       
    }
    last_btn = btn;

  manual steering of resistance using BOOT button
        if (elite_ready) {     //elite_ready
            int btn = gpio_get_level(BOOT_PIN);
            if (btn == 0 && last_btn == 1)
                btn_press_time = xTaskGetTickCount();
